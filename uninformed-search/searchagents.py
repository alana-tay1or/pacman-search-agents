...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        cornerFlags = [False, False, False, False]
        if self.startingPosition in self.corners:
            index = self.corners.index(self.startingPosition)
            cornerFlags[index] = True
        return (self.startingPosition, tuple(cornerFlags))
    ...
    def isGoalState(self, state):
        ...
        return corners_visited.count(True) == 4
...
def cornersHeuristic(state, problem):
    ...
    goals = [c for i, c in enumerate(corners) if not corners_visited[i]]
    if not goals:
        return 0

    h = max(util.manhattanDistance(position, g) for g in goals)
